...
\newcommand{\blue}[1]{\textbf{\rlap{\color{blue}#1}}}
...
Z důvodu úspory místa a kapacity přenosového kanálu jsou zaznamenány pouze témata (topics), která jsou uložena v souboru \verb|path_to_workspace/clopema_cvut/clopema_collect_model_data/matlab/topics/topics.txt|.\\ Pro~tento experiment jsem vybral tyto témata (topics):\\
\\
...
\begin{figure}[H]
	\centering  	
  	\includegraphics[width=14cm]{pictures/rgb.eps}
...
\begin{figure}[H]
	\centering  	
  	\includegraphics[width=14cm]{pictures/filtred.eps}
...
\begin{figure}[H]
	\centering  	
  	\includegraphics[width=14cm]{pictures/mask.eps}
...
\begin{itemize}
  	\item \verb|[ centerOfFabric ] = findCenterOfFabric( maskFrontOfRGB )|\\
\\
    		Vstupy do funkce
        		\begin{itemize}
  			\item \verb|maskFrontOfRGB| (\textit{matice buněk}) - obsahuje filtry obrazů RGB a časovou značku původních snímků.
...
\begin{figure}[H]
	\centering  	
  	\includegraphics[width=14cm]{pictures/center.eps}
  	\caption[]{Příklad středu látky v obraze.	  	
  	}
  	\label{fig:centerPic}
\end{figure}
...
\begin{figure}[H]
	\centering  	
  	\includegraphics[width=14cm]{pictures/maskandpoints.eps}
...
\begin{figure}[H]
	\centering  	
  	\includegraphics[width=14cm]{pictures/points.eps}
...
\subsubsection{Nalezení oblastí výskytu pohybující se látky}
\begin{itemize}
	\item \verb|nanmask = depthNanFilter( frontOfDepth )|
	Vstupy do funkce
        		\begin{itemize}
  			\item \verb|XXX| (\textit{XXX}) - XXX.
    			\end{itemize}
    		Výstupy z funkce
        		\begin{itemize}
  			\item \verb|XXX| (\textit{XXX}) - XXX. 
...
\subsubsection{Vzor bodů}
\begin{itemize}
	\item \verb|pointPattern = depthPointsPosition( nanmask, n_points_x, n_points_z, otherMask )|
	Vstupy do funkce
        		\begin{itemize}
  			\item \verb|XXX| (\textit{XXX}) - XXX.
    			\end{itemize}
...
\begin{itemize}
	\item \verb|points  = getDepthValueOfPoints( frontOfDepth, pointPattern, area )|
	Vstupy do funkce
        		\begin{itemize}
  			\item \verb|XXX| (\textit{XXX}) - XXX.
    			\end{itemize}
